        ...
        <template v-slot:top>
            <v-toolbar flat>
                ...
                <v-dialog v-model="dialog" max-width="500px">
                    ...
                    <v-card>
                        <v-card-title>
                            <span>แก้ไขชื่อ Detail Product</span>
                        </v-card-title>
                            ...
                            <v-container>
                                <v-row>
                                    <v-col cols="12" sm="12" md="12">
                                        <v-text-field v-model="editedItem.name" label="ชื่อ Sub product"></v-text-field>
                                    </v-col>     
                                    <v-col cols="12" sm="12" md="12">
...
export default {
    ...
    methods: {
        initialize() {
            ...
            db.ref("detail_product/").on("value", (snapshot) => {                
                let item = [];
                let num = 0;
                this.desserts = [];
                ...
                for (const key in childData) {
                    ...
                    const getproductPromise = db.ref(`product/${product.idClass}/`).once("value");
                    Promise.all([getproductPromise]).then((snapshots)=>{
                        const product_data = snapshots[0].val();                        
                        num += 1;
                        item.push({ name: product.name, 
                            key: product.id, 
                            index: num ,
                            idClass:product.idClass,
                            idClass_name:product_data.name,
        ...
        edit_save() {
            const db = this.$fireModule.database();
            db.ref(`detail_product/${this.editedItem.key}`).update({
